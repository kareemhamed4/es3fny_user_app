...
void main()async{
  ...
  currentLatitude = CacheHelper.getData(key: 'currentLatitude');
  ...
  isBlind = CacheHelper.getData(key: "isBlind") ?? 1;
  bool? onBoarding = CacheHelper.getData(key: "onBoarding");
  Widget widget;
  if(onBoarding != null){
    if(token != null){
      widget = SplashScreen(isBlind: isBlind == 0 ? true : false);
    }else{
      widget = LoginScreen();
    }
  }else {
  ...
  Widget build(BuildContext context) {
    return MultiBlocProvider(
      providers: [
        ...
        BlocProvider(create: (BuildContext context) => ProfileCubit()..getUserData()..getFamilyMember()..startLocationUpdates()),
